...
import { z } from "zod";
import { tool } from "ai";
import { findWorkspaceRootForPath, getActiveWorkspaceRoot } from "../workspace";
  ...
  try {
    ...
    const similar = entries
      .filter((entry) => {
        ...
        return isPathAllowed(workspaceRoot, fullPath, ig) &&
          fs.statSync(fullPath).isFile();
      })
      .map((entry) => ({
        name: entry,
        dist: levenshtein(entry.toLowerCase(), targetLower),
      }))
      .filter((item) => item.dist <= 3 && item.dist > 0)
...
function readFileInternal(
  ...
  try {
    ...
    if (!fs.existsSync(resolvedPath)) {
      const suggestions = findSimilarFiles(
      ...
      if (suggestions.length > 0) {
        const relativeSuggestions = suggestions.map((s) =>
          path.relative(workspaceRoot, path.join(path.dirname(resolvedPath), s))
        );
        return `Error: File not found: ${resolvedPath}. Did you mean: ${
          relativeSuggestions.join(", ")
        }?`;
      } else {
    ...
    return `<file name="${resolvedPath}" start_line="${
      startLine + 1
    }" end_line="${endLine}" full_length="${lines.length}">\n${
      numberedLines.join("\n")
    }\n</file>`;
  } catch (error) {
...
export const readFileTool = tool({
  description:
    `Reads one or more files at the specified paths. Returns file contents with line numbers.
...
- Lines longer than 2000 chars will be truncated`,
  inputSchema: z.object({
    ...
    ])).describe("Array of file read requests - can be strings or objects"),
  }),
  execute: async (args) => {
    try {
      // Handle both old and new formats for backward compatibility
      let files: Array<{ file_path: string; offset?: number; limit?: number }> =
        [];
      if (Array.isArray(args.files)) {
        files = args.files.map((entry) => {
          if (typeof entry === "string") {
            return { file_path: entry };
          } else if (typeof entry === "object" && entry.file_path) {
            return entry;
          } else {
            throw new Error(
              `Invalid file entry: ${
                JSON.stringify(entry)
              }. Expected string or {file_path: string, ...}`,
            );
          }
        });
      } else {
        throw new Error(
          `Invalid files argument: ${
      ...
      return `Tool call failed: Invalid arguments shape. Expected: { files: ["path1", "path2"] or [{ file_path: "path", offset?: number, limit?: number }] }. You provided: ${
        JSON.stringify(args)
...
export const listDirTool = tool({
  description:
    `Lists files and directories for one or more paths. Returns relative paths with sizes.
- Provide an array of directories (use a single-item array for one directory)`,
  inputSchema: z.object({
    directories: z.array(
      z.string().describe("Absolute path to the directory to list"),
    ),
  }),
    ...
    for (const DirectoryPath of directories) {
      try {
        ...
        if (!isPathAllowed(workspaceRoot, resolvedDir, ig)) {
          outputs.push(
            `Error: Directory is outside workspace or ignored: ${DirectoryPath}`,
          );
          continue;
        }
...
export const grepSearchTool = tool({
  description:
    `A powerful search tool. Searches for patterns in files within directories.
  ...
  execute: async ({ searches }) => {
    ...
    for (const search of searches) {
      ...
      try {
        ...
        const ig = loadGitignore(workspaceRoot);
        const resolvedSearchPath = resolveWorkspacePath(
          workspaceRoot,
          SearchPath,
        );
        if (!isPathAllowed(workspaceRoot, resolvedSearchPath, ig)) {
          outputs.push(
            `Error: Search path is outside workspace or ignored: ${SearchPath}`,
          );
          continue;
        }
        ...
        if (results.length === 0) {
          outputs.push(
            `No matches found for "${Query}" in ${resolvedSearchPath}`,
          );
          continue;
        }
...
export const findByNameTool = tool({
  description:
    `Search for files and subdirectories by name pattern (glob format).
- Provide an array of searches (use a single-item array for one search).`,
  inputSchema: z.object({
    searches: z.array(z.object({
  ...
  execute: async ({ searches }) => {
    ...
    for (const search of searches) {
      ...
      try {
        ...
        const ig = loadGitignore(workspaceRoot);
        const resolvedSearchDir = resolveWorkspacePath(
          workspaceRoot,
          SearchDirectory,
        );
        if (!isPathAllowed(workspaceRoot, resolvedSearchDir, ig)) {
          outputs.push(
            `Error: Search directory is outside workspace or ignored: ${SearchDirectory}`,
          );
          continue;
        }
        const results: string[] = [];
        ...
        if (results.length === 0) {
          outputs.push(
            `Found 0 results for pattern "${Pattern}" in ${resolvedSearchDir}`,
          );
          continue;
        }
