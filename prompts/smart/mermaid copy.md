...
graph TB
    subgraph trace1 [Request Handling]
        ...
        t1a_handle -->|"validates"| t1b_validate
        t1a_handle -->|"routes"| t1c_route
    end
    ...
    subgraph trace2 [Data Processing]
        ...
        t2a_process -->|"transforms"| t2b_transform
        t2a_process -->|"persists"| t2c_persist
    end

    %% Shared infrastructure (used by both traces)
    config["Config"]
    t1a_handle -.->|"loads"| config
    t2a_process -.->|"loads"| config

    %% Cross-trace connection
    t1c_route -.->|"invokes"| t2a_process
```
