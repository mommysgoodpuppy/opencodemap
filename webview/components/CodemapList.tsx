...
import { Circle, RefreshCw, Trash2, X } from "lucide-react";
...
export const CodemapList: React.FC<CodemapListProps> = ({
  ...
  const [lastProgressSnapshot, setLastProgressSnapshot] = useState<
  ...
  const buildMetaTitle = (item: CodemapHistoryItem) => {
    ...
    if (typeof meta.totalTokens === "number") {
      parts.push(`Tokens: ${meta.totalTokens.toLocaleString()}`);
    }
    if (meta.timeTakenMs) {
      parts.push(`Time: ${formatDuration(meta.timeTakenMs)}`);
    }
    ...
    if (meta.git?.dirty !== undefined) {
      parts.push(`Dirty: ${meta.git.dirty ? "yes" : "no"}`);
    }
    return parts.length ? parts.join(" • ") : undefined;
  };

  const isCurrentCodemap = (item: CodemapHistoryItem) => {
    if (currentCodemapId) return currentCodemapId === item.id;
  ...
  const showCompletion = !isProcessing && Date.now() < completionHoldUntil &&
    lastProgressSnapshot;
  const displayProgress = showCompletion ? lastProgressSnapshot : progress;
  const displayPercent = displayProgress
    ? Math.round(
      (displayProgress.completedStages / displayProgress.totalStages) * 100,
    )
    : 0;
  const displayComplete = displayPercent >= 100;

  // Get current active agent label
  const currentAgentLabel = progress?.activeAgents[activeAgentIndex]?.label ||
    progress?.currentPhase || "Processing...";
  const lastFileName = progress?.lastFile
    ? progress.lastFile.split(/[\\/]/).pop()
    : "";
  ...
  const showTps = Boolean(
    !isStage1 && progress?.totalTokens !== undefined &&
      progress.totalTokens > 0,
  );
  const showFire = Boolean(
    !isStage1 && (progress?.parallelToolsActive || tps >= tpsFireThreshold) &&
      isProcessing,
  );
  ...
  useEffect(() => {
    if (!isProcessing && progressComplete && progress) {
      ...
      setLastProgressSnapshot({
        ...progress,
        completedStages: progress.totalStages,
      });
  ...
  return (
    <div className="codemap-list-section">
      ...
      {(isProcessing || showCompletion) && (
        <div
          className={`codemap-item processing-item ${
            showFire ? "parallel-fire" : ""
          } ${tpsBoost ? "tps-boost" : ""}`}
          style={{
            ...(showFire
              ? {
        ...
        >
            ...
            <div className="progress-bar">
              <div
                className={`progress-fill${
                  displayComplete ? " progress-complete" : ""
                }`}
          ...
          <div className="progress-status">
            ...
            <div className="status-activity">
              {displayProgress?.totalToolCalls !== undefined &&
                displayProgress.totalToolCalls > 0 && (
              ...
              {showTps && (
                <div className="tps-indicator" title="Tokens per second">
                  <div className="tps-gauge">
                    <span
                      className="tps-needle"
                      style={{
                        transform: `translateX(-50%) rotate(${tpsAngle}deg)`,
                      }}
                    />
                    <span className="tps-center" />
                  </div>
                  <span className="tps-value">
                    {Math.round(tpsClamped)} tps
                  </span>
                </div>
              )}

              {displayProgress?.totalTokens !== undefined &&
                displayProgress.totalTokens > 0 && (
                <div className="token-counter">
                  <div className="token-stream">
                    <div
                      className={`token-particle ${
                        displayProgress.totalTokens % 3 === 0 ? "active" : ""
                      }`}
                    />
                    <div
                      className={`token-particle ${
                        displayProgress.totalTokens % 3 === 1 ? "active" : ""
                      }`}
                    />
                    <div
                      className={`token-particle ${
                        displayProgress.totalTokens % 3 === 2 ? "active" : ""
                      }`}
                    />
                  </div>
                  <span>
                    {displayProgress.totalTokens.toLocaleString()} tokens
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="progress-meta">
            {displayProgress?.stageNumber !== undefined && (
              <div className="meta-chip" title="Current stage">
                ...
                <span className="meta-value">
                  {displayProgress.stageNumber}
                </span>
              </div>
            )}
            {displayProgress?.filesRead !== undefined &&
              displayProgress.filesRead > 0 && (
            ...
            {displayProgress?.linesRead !== undefined &&
              displayProgress.linesRead > 0 && (
              <div className="meta-chip" title="Lines read">
                <span className="meta-label">Lines</span>
                <span className="meta-value">
                  {displayProgress.linesRead.toLocaleString()}
                </span>
              </div>
            )}
            {displayProgress?.toolBreakdown &&
              (displayProgress.toolBreakdown.internal +
                  displayProgress.toolBreakdown.vscode) > 0 &&
              (
                <div
                  className="meta-chip"
                  title="Tools used (internal / VS Code)"
                >
                  <span className="meta-label">Tools</span>
                  <span className="meta-value">
                    {displayProgress.toolBreakdown.internal}/{displayProgress
                      .toolBreakdown.vscode}
                  </span>
                </div>
              )}
          </div>
          ...
          {displayProgress?.lastFile && (
            <div className="progress-file" title={displayProgress.lastFile}>
              Reading:{" "}
              <span className="mono">
                {lastFileName || displayProgress.lastFile}
              </span>
            </div>
          )}
      ...
      {filteredHistory.length === 0 && !isProcessing
        ...
        : (
          filteredHistory.map((item) => (
            <div
              key={item.id}
              className={`codemap-item ${
                isCurrentCodemap(item) ? "active" : ""
            ...
            >
              ...
              <div className="codemap-item-time">
